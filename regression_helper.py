...
def plot_all_J(X, y):
    ...
    k = np.linspace(175000, 195000, 100)
...
def lin_grad(X, y, k_init, alpha, iters=20):
    ...
    plt.ylabel("Значение средней квадратичной ошибки")
    plt.xlabel("Значение коэфициента")
    T = np.linspace(175000, 195000, 100)
...
def Traice(X, y):
    k_init = IntSlider(min=176000, max=194000, step=2000, value=176000, description='K init:')
